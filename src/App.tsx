...
export default function App() {
  ...
  const [avatar, setAvatar] = useState("https://picsum.photos/seed/zhaqi/400/400");
      ...
      <AnimatePresence>
        {activeContact && (
          <motion.div
            ...
            className="fixed inset-0 z-[200] bg-bg/80 backdrop-blur-md flex items-center justify-center p-6"
          >
            <motion.div
              ...
              className="glass p-12 rounded-3xl max-w-sm w-full relative overflow-hidden group"
            >
              ...
              <div className="text-center">
                ...
                <h3 className="text-2xl font-black mb-8">{activeContact.value}</h3>
        ...
        {isGeneralContactModalOpen && (
          ...
          >
            <motion.div
              ...
              exit={{ scale: 0.9, y: 20 }}
              onClick={(e) => e.stopPropagation()}
              className="glass p-12 rounded-3xl max-w-lg w-full relative overflow-hidden"
            >
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-brand to-transparent" />
              ...
              <div className="mb-12 text-center">
                <span className="text-[10px] font-mono text-brand uppercase tracking-[0.4em] mb-4 block">GET IN TOUCH</span>
                <h2 className="text-4xl font-black">联系方式</h2>
              </div>
              ...
              <div className="space-y-4">
                {CONTACTS.map((contact, i) => (
                  <div 
                    key={i}
                    className="glass flex items-center justify-between p-6 rounded-2xl group hover:bg-white/5 transition-all"
                  >
                    <div className="flex items-center gap-6">
                      <div className="w-12 h-12 rounded-xl bg-brand/10 flex items-center justify-center group-hover:bg-brand group-hover:text-bg transition-all">
                        <contact.icon className="w-6 h-6" />
                      </div>
                      <div>
                        <span className="text-[10px] font-mono text-white/30 uppercase tracking-widest block mb-1">{contact.label}</span>
                        <span className="text-lg font-black">{contact.value}</span>
                      </div>
                    </div>
                    <button 
                      onClick={() => {
          ...
          <div className="relative w-32 h-32 md:w-48 md:h-48 mb-6 md:mb-12 group">
            ...
            <div className="relative w-full h-full rounded-full overflow-hidden border-2 border-white/10 z-10">
              <img 
                ...
                referrerPolicy="no-referrer"
              />
              <label className="absolute inset-0 flex items-center justify-center bg-bg/60 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
